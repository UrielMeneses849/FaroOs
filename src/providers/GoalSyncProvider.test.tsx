...
import { GoalSyncProvider } from './GoalSyncProvider'
...
vi.mock('../repositories/goalRepository', () => ({
  isSupabaseGoalId: (id: string) =>
    /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id),
  goalRepository: repositoryMocks,
}))
...
describe('GoalSyncProvider', () => {
  it('hidrata y replica altas, cambios y eliminaciones en orden', async () => {
    ...
    useFaroStore.getState().createGoal({
      id,
      title: 'Objetivo sincronizado',
      area: 'personal',
      status: 'active',
      projectIds: [],
      createdAt: timestamp,
    ...
    useFaroStore.getState().updateGoal(id, { status: 'completed' })
    await waitFor(() =>
    ...
    useFaroStore.getState().deleteGoal(id)
    await waitFor(() =>
