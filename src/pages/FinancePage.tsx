...
import { fortnightPeriodForDate } from '../services/financeBudgetCycle'
...
export function FinancePage() {
  ...
  return <div className={`page finance-os finance-os--${panel}`}>
    ...
    {panel === 'recurring' && <div className="finance-expenses-grid">
      ...
      <section className="finance-expense-panel">
        <SectionHead eyebrow="Una sola ocasión" title="Gastos eventuales" action="+" actionLabel="Registrar eventual" onClick={() => startMovement({ type: 'expense', status: 'planned' })} />
    ...
    {dialog === 'recurring' && <RecurringDialog
      ...
      onSave={async (item) => {
        ...
        if (editingRecurring && recurringEditMode === 'period') {
          await financeRecurringOccurrenceRepository.savePeriod(item.id, selectedPeriod, item.nextOccurrence, item.amountCents, user.id, item.description)
        ...
        await finish(editingRecurring ? recurringEditMode === 'period' ? 'Periodo recurrente actualizado.' : 'Recurrencia actualizada.' : 'Movimiento recurrente guardado.')
      }}
    />}
    ...
    {dialog === 'goalDetail' && selectedGoal && <GoalDetailModal goal={selectedGoal} data={data} onClose={()=>{setDialog(null);setSelectedGoal(undefined)}} onAdd={()=>{setSelectedGoalItem(undefined);setDialog('goalItem')}} onEdit={item=>{setSelectedGoalItem(item);setDialog('goalItem')}} onPurchase={item=>{setSelectedGoalItem(item);setDialog('purchaseItem')}} onDiscard={async item=>{if(!user)return;await financePlanningRepository.saveGoalItem({...item,status:'discarded'},user.id);await refresh();setFeedback('Artículo descartado.')}} onDelete={item=>setDeletingGoalItem(item)} />}
...
function GoalDetailModal({goal,data,onClose,onAdd,onEdit,onPurchase,onDiscard,onDelete}:{goal:FinanceGoal;data:FinanceData;onClose:()=>void;onAdd:()=>void;onEdit:(item:FinanceGoalItem)=>void;onPurchase:(item:FinanceGoalItem)=>void;onDiscard:(item:FinanceGoalItem)=>Promise<void>;onDelete:(item:FinanceGoalItem)=>void}) {
  const progress=goalProgress(goal.id,data);const target=goalTargetCents(goal.id,data);const spent=goalSpentCents(goal.id,data);const available=goalAvailableCents(goal.id,data)
  const items=data.goalItems.filter(item=>item.goalId===goal.id)
  return <Modal open panelClassName="finance-goal-detail-modal" title={`Detalle · ${goal.name}`} onClose={onClose}><div className="finance-goal-detail">
    <div className="finance-goal-detail__summary"><span>Objetivo<strong>{formatMxn(target)}</strong></span><span>Aportado<strong>{formatMxn(progress.savedCents)}</strong></span><span>Gastado<strong>{formatMxn(spent)}</strong></span><span>Disponible<strong>{formatMxn(available)}</strong></span><span>Por comprar<strong>{formatMxn(Math.max(0,target-spent))}</strong></span><span>Por financiar<strong>{formatMxn(Math.max(0,target-progress.savedCents))}</strong></span></div>
    <div className="finance-goal-detail__heading"><div><span className="eyebrow">Artículos</span><strong>{items.length} registrados</strong></div><Button size="sm" icon={<Plus size={14}/>} onClick={onAdd}>Añadir artículo</Button></div>
    <div className="finance-goal-item-list">{items.map(item=>{const movement=data.transactions.find(transaction=>transaction.id===item.transactionId);return <article key={item.id} onClick={()=>onEdit(item)}><div><strong>{item.name}</strong><small>{item.url?<a href={item.url} target="_blank" rel="noreferrer" onClick={event=>event.stopPropagation()}>Abrir enlace <ArrowUpRight size={11}/></a>:'Sin enlace'}</small></div><b>{formatMxn(item.priceCents)}</b><span>{item.priority}</span><span>{item.status}</span><small>{item.purchaseDate??'Sin compra'}</small><small title={movement?.description}>{movement?`Movimiento: ${movement.description}`:'Sin movimiento'}</small><nav onClick={event=>event.stopPropagation()}><button onClick={()=>onEdit(item)}>Editar</button>{item.status==='pending'&&<button onClick={()=>onPurchase(item)}>Comprar</button>}{item.status!=='discarded'&&<button onClick={()=>void onDiscard(item)}>Descartar</button>}<button className="danger" onClick={()=>onDelete(item)}>Eliminar</button></nav></article>})}{!items.length&&<EmptyState title="Sin artículos" description="Añade el primer artículo de esta meta." action={<Button onClick={onAdd}>Añadir artículo</Button>}/>}</div>
  </div></Modal>
}

function GoalItemDialog({goal,initial,onClose,onSave}:{goal:FinanceGoal;initial?:FinanceGoalItem;onClose:()=>void;onSave:(item:Omit<FinanceGoalItem,'createdAt'|'updatedAt'>)=>Promise<void>}){const[name,setName]=useState(initial?.name??'');const[price,setPrice]=useState(initial?String(initial.priceCents/100):'');const[priority,setPriority]=useState<FinanceGoalItem['priority']>(initial?.priority??'medium');const[url,setUrl]=useState(initial?.url??'');const[notes,setNotes]=useState(initial?.notes??'');const[status,setStatus]=useState<FinanceGoalItem['status']>(initial?.status??'pending');const[error,setError]=useState('');return <Modal open title={`Artículo · ${goal.name}`} onClose={onClose}><form className="finance-form" onSubmit={async event=>{event.preventDefault();if(!name.trim()||moneyToCents(price)<=0)return setError('Completa nombre y precio.');try{await onSave({id:initial?.id??crypto.randomUUID(),goalId:goal.id,name:name.trim(),priceCents:moneyToCents(price),priority,url:url||undefined,notes:notes||undefined,status,purchaseDate:initial?.purchaseDate,transactionId:initial?.transactionId})}catch(reason){setError(reason instanceof Error?reason.message:'No se pudo guardar.')}}}><label>Nombre<input autoFocus required value={name} onChange={event=>setName(event.target.value)}/></label><div className="finance-form-grid"><label>Precio<input type="number" min=".01" step=".01" required value={price} onChange={event=>setPrice(event.target.value)}/></label><label>Prioridad<select value={priority} onChange={event=>setPriority(event.target.value as typeof priority)}><option value="low">Baja</option><option value="medium">Media</option><option value="high">Alta</option><option value="critical">Crítica</option></select></label></div><label>Enlace opcional<input type="url" value={url} onChange={event=>setUrl(event.target.value)}/></label><label>Notas <span>opcional</span><textarea rows={3} value={notes} onChange={event=>setNotes(event.target.value)}/></label><label>Estado<select value={status} onChange={event=>setStatus(event.target.value as typeof status)}><option value="pending">Pendiente</option><option value="purchased">Comprado</option><option value="discarded">Descartado</option></select></label><ErrorText value={error}/><div className="modal-actions"><Button type="button" variant="ghost" onClick={onClose}>Cancelar</Button><Button type="submit">Guardar</Button></div></form></Modal>}
...
function BudgetDialog({ month, categories, onClose, onSave }: { month: string; categories: { id: string; name: string }[]; onClose: () => void; onSave: (item: { categoryId: string; month: string; plannedAmountCents: number;name:string;periodStart:string;periodEnd:string;carryOverEnabled:boolean }) => Promise<void> }) {
  const [categoryId, setCategoryId] = useState(categories.find(item=>item.name==='Personal')?.id??categories[0]?.id ?? ''); const [amount, setAmount] = useState(''); const [error, setError] = useState(''); const selected = new Date(); const base = parseISO(month); const sameMonth = selected.getFullYear() === base.getFullYear() && selected.getMonth() === base.getMonth(); const period = fortnightPeriodForDate(sameMonth ? format(selected, 'yyyy-MM-dd') : month)
  const submit = async (event: FormEvent) => { event.preventDefault(); const result = financeBudgetSchema.safeParse({ categoryId, month: period.month, plannedAmountCents: moneyToCents(amount) }); if (!result.success) return setError(result.error.issues[0]?.message ?? 'Revisa los campos.'); try { await onSave({...result.data,name:'Gastos Personales',periodStart:period.periodStart,periodEnd:period.periodEnd,carryOverEnabled:false}) } catch (reason) { setError(reason instanceof Error ? reason.message : 'No se pudo guardar.') } }
  return <Modal open title="Presupuesto quincenal" onClose={onClose}><form className="finance-form" onSubmit={submit}><p className="finance-form-hint">Periodo: {period.periodStart} — {period.periodEnd}</p><label>Categoría vinculada<select value={categoryId} onChange={(event) => setCategoryId(event.target.value)}>{categories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>Monto límite MXN<input autoFocus type="number" min="0" step=".01" required value={amount} onChange={(event) => setAmount(event.target.value)} /></label><ErrorText value={error} /><div className="modal-actions"><Button type="button" variant="ghost" onClick={onClose}>Cancelar</Button><Button type="submit">Guardar</Button></div></form></Modal>
}
...
function RecurringDialog({ initial, editMode = 'period', occurrence, period, suggestedDate, presetType, accounts, categories, onClose, onSave }: {
  ...
  const [description, setDescription] = useState(initial?.description ?? '')
  const [amount, setAmount] = useState(String(((editMode === 'template' ? initial?.amountCents : occurrence?.amountCents) ?? initial?.amountCents ?? 0) / 100 || ''))
  ...
  const [date, setDate] = useState(editMode === 'template' ? initial?.nextOccurrence ?? suggestedDate : occurrence?.expectedDate ?? suggestedDate)
  ...
  if (initial && editMode === 'period') {
    return <Modal open title={`Editar ${periodLabel}`} onClose={onClose}><form className="finance-form" onSubmit={submit}>
      <label>Concepto de {periodLabel}<input value={description} onChange={(event) => setDescription(event.target.value)} /></label>
  ...
  return <Modal open title={initial ? 'Editar recurrencia' : 'Nuevo recurrente'} onClose={onClose}><form className="finance-form" onSubmit={submit}>
