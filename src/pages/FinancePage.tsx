...
export function FinancePage() {
  ...
  return <div className="page finance-os">
    ...
    {panel === 'budgets' && <section className="finance-section"><SectionHead eyebrow="Control quincenal" title="Gastos Personales" action="Definir presupuesto" onClick={() => setDialog('budget')} />{budgets.length ? <div className="finance-budget-list">{budgets.map((budget) => <article key={budget.id} className={budget.usedPercentage > 100 ? 'exceeded' : budget.usedPercentage >= 85 ? 'warning' : budget.usedPercentage >= 70 ? 'attention' : ''}><div><strong>{budget.name ?? 'Gastos Personales'}</strong><span>{budget.periodStart} — {budget.periodEnd}</span></div><ProgressBar value={Math.min(100, budget.usedPercentage)} /><footer><span>{budget.usedPercentage.toFixed(0)}% utilizado · {budget.usedPercentage > 100 ? 'Excedido' : budget.usedPercentage >= 100 ? 'Límite alcanzado' : budget.usedPercentage >= 85 ? 'Alerta 85%' : budget.usedPercentage >= 70 ? 'Atención 70%' : 'En control'}</span><b>{formatMxn(budget.remainingCents)} restante</b></footer></article>)}</div> : <EmptyState title="Sin presupuesto quincenal" description="Define el límite de Gastos Personales para esta quincena." action={<Button onClick={() => setDialog('budget')}>Crear presupuesto</Button>} />}</section>}
    ...
    {panel === 'fund' && <section className="finance-section finance-fund"><SectionHead eyebrow="Reserva sin objetivo" title="Fondo de Ahorro" action="Registrar movimiento" onClick={() => setDialog('fundEntry')} /><div className="finance-fund-kpis"><FinanceKpi label="Saldo reservado" value={fundMetrics.balanceCents} context="Reduce sólo el disponible operativo" /><FinanceKpi label="Aportado este mes" value={fundMetrics.monthCents} /><FinanceKpi label="Aportado este año" value={fundMetrics.yearCents} /><FinanceKpi label="Última aportación" value={fundMetrics.lastEntry?.amountCents ?? 0} context={fundMetrics.lastEntry?.entryDate ?? 'Sin movimientos'} /></div><div className="finance-fund-history">{data.savingsFundEntries.map(item=><article key={item.id}><time>{item.entryDate}</time><span>{item.description??(item.amountCents>0?'Aportación':'Retiro')}</span><strong className={item.amountCents>=0?'positive':'negative'}>{formatMxn(item.amountCents)}</strong></article>)}{!data.savingsFundEntries.length&&<EmptyState title="Fondo vacío" description="Reserva dinero sin asignarlo a una meta específica." />}</div></section>}
