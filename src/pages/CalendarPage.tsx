...
import { normalizeCalendarData } from '../services/calendarService'
...
export function CalendarPage() {
  ...
  const events: EventInput[] = filtered.map((item) => ({
    ...
    allDay: item.allDay,
    editable: item.editable, durationEditable: item.sourceType === 'task' && !item.allDay,
  ...
  const drop = (info: EventDropArg) => {
    ...
    if (item.sourceType !== 'task' || !info.event.start) return info.revert()
    ...
    void persistMove(item, dueAt, tasks.find((task) => task.id === item.sourceId)?.estimatedMinutes, info.revert)
  }
  const resize = (info: EventResizeDoneArg) => {
    const item = info.event.extendedProps.item as CalendarItem
    if (item.sourceType !== 'task' || !info.event.start || !info.event.end) return info.revert()
  ...
  const refreshFromStore = () => setLocalItems(normalizeCalendarData({ tasks: useFaroStore.getState().tasks, projects: useFaroStore.getState().projects, goals: useFaroStore.getState().goals }))
