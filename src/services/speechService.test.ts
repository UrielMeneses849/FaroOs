...
describe('speechService', () => {
  ...
  it('conserva el status, código y mensaje seguros del proveedor',async()=>{const {service,fetch}=setup();fetch.mockResolvedValueOnce(new Response(JSON.stringify({error:'speech_provider_failed',providerStatus:400,providerCode:'voice_not_found',providerMessage:'Voice not found.'}),{status:400,headers:{'Content-Type':'application/json'}}));const error=await service.speak('Prueba').catch(cause=>cause);expect(error).toBeInstanceOf(SpeechProviderError);expect(error).toMatchObject({providerStatus:400,providerCode:'voice_not_found',providerMessage:'Voice not found.'})})
