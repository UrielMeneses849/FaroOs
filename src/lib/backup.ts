...
export const faroDataSchema: z.ZodType<FaroData> = z.object({
  ...
  healthLogs: z.array(z.object({ ...base, area: z.literal('health'), occurredAt: z.string(), energy: z.number().min(1).max(10), mood: z.number().min(1).max(10).optional(), anxiety: z.number().min(1).max(10).optional(), sleepHours: z.number().nonnegative().max(24).optional(), movementMinutes: z.number().nonnegative().optional(), trainingMinutes: z.number().nonnegative().optional(), meditationMinutes: z.number().nonnegative().optional(), weightKg: z.number().positive().optional(), waterLiters: z.number().nonnegative().optional(), notes: z.string().optional() })),
