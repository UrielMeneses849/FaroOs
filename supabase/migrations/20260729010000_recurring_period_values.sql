-- Recurring templates describe the concept; each monthly occurrence owns amount and date.

alter table public.finance_recurring_occurrences
  add column if not exists amount numeric(14,2)
    check (amount is null or amount > 0);

update public.finance_recurring_occurrences occurrence
set amount = template.amount
from public.finance_recurring_transactions template
where occurrence.recurring_transaction_id = template.id
  and occurrence.user_id = template.user_id
...
begin
  ...
  if not found then
    raise exception 'Define el monto y la fecha de este periodo antes de registrarlo';
  end if;
  if occurrence.amount is null or occurrence.amount <= 0 then
    raise exception 'Define un monto válido para este periodo';
  end if;
  if occurrence.expected_date <> target_expected_date then
    raise exception 'La fecha no coincide con la configuración del periodo';
  end if;
  ...
  insert into public.finance_transactions (
    ...
    occurrence.amount, occurrence.expected_date, template.description, 'completed', template.id
  );
