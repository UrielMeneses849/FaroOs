...
import { accessToken, authenticatedUser, decryptRefreshToken, GoogleApiError, googleJson, serviceClient } from '../_shared/googleCalendar.ts'
import { googleEventsUrl, maskCalendarId } from '../_shared/googleCalendarPure.ts'

type Connection = { id:string;user_id:string;google_account_email:string|null;calendar_id:string|null;calendar_name:string|null;encrypted_refresh_token:string;refresh_token_iv:string;connected_at:string;last_synced_at:string|null;status:string }
...
const publicConnection = (connection: Connection | null) => connection ? ({
  ...
  lastSyncedAt: connection.last_synced_at, status: connection.status,
...
Deno.serve(async (request) => {
  ...
  if (action === 'disconnect') {
    if (connection) await db.from('google_calendar_connections').delete().eq('user_id', user.id)
  ...
  if (action === 'list' || action === 'select') {
    ...
    return json({ connection: { ...publicConnection(connection), calendarId:selected.id, calendarName:selected.name, accountEmail:primary?.id, status:'active' } })
  }
  if (action === 'events') {
